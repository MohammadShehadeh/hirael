...
import { Eye, EyeOff, Lock, LockOpen, Plus, Search, Trash2, Upload } from 'lucide-react';
...
import { Field, FieldError } from '@/registry/hirael/bases/radix/ui/field';
...
const EnvEditorContext = React.createContext<EnvEditorContextValue | null>(null);
...
interface EnvEditorProps extends Omit<React.ComponentProps<'div'>, 'defaultValue'> {
  value?: EnvVar[];
  defaultValue?: EnvVar[];
...
const EnvEditor = ({
  ...
  const ctx = React.useMemo<EnvEditorContextValue>(
    ...
    [vars, visible, query, errors, dirtyCount, adding, update, remove, add, save, discard],
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <EnvEditorImport />
        <Button type="button" size="sm" data-slot="env-editor-add-trigger" onClick={() => setAdding(true)}>
          <Plus aria-hidden />
          Add variable
        </Button>
        {children}
...
const EnvEditorImport = ({ className, children = 'Import .env', ...props }: EnvEditorImportProps) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Paste the file contents. Comments and blank lines are ignored; existing keys are updated in place.
          </DialogDescription>
        </DialogHeader>
        <Textarea
          value={text}
...
const ROW_GRID = 'md:grid md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_auto_auto] md:items-start md:gap-3';
...
interface EnvEditorRowProps extends Omit<React.ComponentProps<'div'>, 'children'> {
  index: number;
}
...
const EnvEditorAdd = ({ className, ...props }: EnvEditorAddProps) => {
  ...
  const [environments, setEnvironments] = React.useState<EnvEnvironment[]>(['production', 'preview', 'development']);
  const [touched, setTouched] = React.useState(false);
  const keyErrorId = React.useId();

  if (!adding) return null;

  const keyError = validateKey(key) ?? (vars.some((v) => v.key === key) ? 'That key already exists.' : null);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center md:h-8">
        <EnvironmentChips label="Environments for the new variable" value={environments} onChange={setEnvironments} />
      </div>
...
const EnvEditorFooter = ({ className, children, ...props }: EnvEditorFooterProps) => {
  ...
  return (
    <div
      ...
      className={cn('flex items-center justify-between gap-3 border-t border-border bg-muted/40 px-4 py-3', className)}
      {...props}
    >
      <span
        aria-live="polite"
      ...
      <div className="flex items-center gap-2">
        ...
        <Button type="button" size="sm" disabled={!dirty || hasErrors} onClick={save}>
          Save
        </Button>
      </div>
    </div>
