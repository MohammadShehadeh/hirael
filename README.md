...
`registry/msh-ui/blocks/` and are listed in `registry.json`. Browse them
...
├── registry/
│   └── msh-ui/                   # canonical source for every registry item
...
└── components.json               # shadcn config; `ui` alias → registry/msh-ui/ui
```
