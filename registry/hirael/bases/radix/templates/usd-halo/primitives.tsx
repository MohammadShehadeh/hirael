...
interface PillButtonProps extends Omit<ComponentProps<'button'>, 'children'> {
  ...
  large?: boolean;
  /** White pill with a dark arrow, for dark surfaces. */
  inverted?: boolean;
}
...
export const Marquee = ({ brands, trackClass, keyframesName, durationSeconds, itemClass }: MarqueeProps) => {
  return (
    <>
      <style>{`
      ...
      <div className={trackClass}>
        {[...brands, ...brands].map((brand, index) => (
          <span key={`${brand.name}-${index}`} className={itemClass} style={brand.style}>
            {brand.name}
          </span>
        ))}
      </div>
