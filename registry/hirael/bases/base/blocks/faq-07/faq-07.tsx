...
const Faq07 = () => {
  return (
    ...
    >
      <div data-slot="faq-intro" className="flex flex-col gap-4 px-6 pb-6 pt-12 md:border-e md:border-border">
        <h2 className="font-serif text-3xl font-medium tracking-tight md:text-4xl">Questions, answered</h2>
        <p className="max-w-md text-sm text-muted-foreground md:text-base">
          The short version of what teams ask before moving their pipelines over.
        </p>
      </div>
      <div className="md:pt-12">
        <Accordion type="single" collapsible className="rounded-none border-y border-border">
          {QUESTIONS.map((q) => (
            <AccordionItem key={q.value} value={q.value} className="px-6">
              <AccordionTrigger className="py-4">{q.question}</AccordionTrigger>
              <AccordionContent className="pb-4 text-muted-foreground">{q.answer}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      </div>
