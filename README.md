...
```bash
npx shadcn@latest add https://msh-ui.dev/r/multi-select.json
```
...
- **Modern shadcn polish.** 1px soft borders, 0.65rem radius scale
  (sm/md/lg/xl derived from `--radius`), zinc neutrals, a light-first
  `:root` with a full `.dark` mode (shadcn convention), no chromatic
  accent — primary draws the eye. Geist Sans for body, Geist Mono
  reserved for code and identifiers.
...
`registry/new-york/blocks/` and are listed in `registry.json`. Browse them
at [msh-ui.dev/blocks](https://msh-ui.dev/blocks).
...
├── registry/
│   └── new-york/                 # canonical source for every registry item
│       ├── ui/                   # primitives + components the registry ships
│       ├── blocks/<block>/       # marketing blocks
│       ├── examples/             # per-component demos: <name>-demo.tsx
...
└── components.json               # shadcn config; `ui` alias → registry/new-york/ui
```
...
3. A consumer runs `npx shadcn add https://msh-ui.dev/r/<name>.json`.
   The shadcn CLI fetches each `registryDependency` from the upstream
   shadcn registry, installs the listed npm dependencies, and copies the
...
Light is the default canvas (`:root`); dark is a faithful inverse applied
via the `.dark` class — the shadcn convention. The palette is pure zinc
(achromatic OKLch) with no warm accent — `--primary` carries emphasis.
Borders are 1px and soft (oklch(0.922) in light, 10% white in dark). Radii
follow shadcn's standard scale derived from
...
The canonical deployment runs at [msh-ui.dev](https://msh-ui.dev).
