...
const CountdownTimerDemo = () => {
  ...
  const [launchTarget] = React.useState(() => Date.now() + (3 * 86400 + 7 * 3600 + 24 * 60 + 30) * 1000);
  const [saleTarget] = React.useState(() => Date.now() + (5 * 3600 + 32 * 60 + 10) * 1000);
  const [shortTarget, setShortTarget] = React.useState(() => Date.now() + 10_000);
    ...
    <div className="grid w-full max-w-2xl gap-8">
      ...
      <div className="grid gap-2">
        ...
        <CountdownTimer
          ...
          completeContent={
            <span className="text-xl font-semibold text-primary">{t({ en: "We're live!", ar: 'انطلقنا!' })}</span>
          }
        />
        {completedAt ? (
