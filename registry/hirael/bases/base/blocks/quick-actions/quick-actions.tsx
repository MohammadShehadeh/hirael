...
type QuickActionProps = useRender.ComponentProps<'button'>;

const QuickAction = ({ className, render, ...props }: QuickActionProps) => {
  return useRender({
    defaultTagName: 'button',
    props: mergeProps<'button'>(
      // Only the default <button> needs the type reset; a rendered element keeps its own.
      render ? {} : { type: 'button' },
      {
        className: cn(
          'group flex flex-col items-start gap-2.5 rounded-lg border border-border bg-card p-3 text-start transition-colors duration-150 hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-popup-open:bg-accent',
          className,
        ),
      },
      props,
      ...
      <QuickActions columns={2} className={cn(ENTER, 'w-full max-w-md')}>
        {QUICK_ACTION_ROWS.map((action) => {
          ...
          return (
            ...
            >
              <PopoverTrigger render={<QuickAction data-done={result ? '' : undefined} />}>
