...
import {
  ...
  InlineEditPreview,
  InlineEditTextarea,
} from '@/registry/hirael/bases/base/components/inline-edit';

const InlineEditDemo = () => {
  const t = useT();
  const [title, setTitle] = React.useState(t({ en: 'Quarterly revenue report', ar: 'تقرير الإيرادات الفصلي' }));
  ...
  return (
    <div className="grid w-full max-w-2xl gap-8">
      ...
      <div className="grid gap-3">
        <p className="text-xs uppercase text-muted-foreground">{t({ en: 'render · heading', ar: 'render · عنوان' })}</p>
        <InlineEdit
          defaultValue={t({
        ...
        >
          <div className="flex items-center gap-2">
            <InlineEditPreview render={<h3 className="text-lg font-semibold tracking-tight" />} />
            <InlineEditInput aria-label={t({ en: 'Edit heading', ar: 'تعديل العنوان' })} />
            <InlineEditControls />
          </div>
        </InlineEdit>
      </div>
