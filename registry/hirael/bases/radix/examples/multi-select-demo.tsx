...
import {
  ...
  MultiSelectContent,
  MultiSelectTrigger,
} from '@/registry/hirael/bases/radix/components/multi-select';
  ...
  return (
    <FieldGroup className="max-w-md gap-8">
      <Field className="gap-2">
        <FieldLabel htmlFor="ms-basic">{t({ en: 'Basic', ar: 'أساسي' })}</FieldLabel>
        <MultiSelect options={FRAMEWORKS} value={basic} onValueChange={setBasic} maxCount={5}>
      ...
      <Field className="gap-2">
        <FieldLabel htmlFor="ms-composed">{t({ en: 'Composed', ar: 'مركّب' })}</FieldLabel>
        <MultiSelect options={FRAMEWORKS} value={composed} onValueChange={setComposed}>
          <MultiSelectTrigger id="ms-composed" placeholder={t({ en: 'Compose your own', ar: 'ركّب اختيارك' })} />
          <MultiSelectContent
            searchPlaceholder={t({
