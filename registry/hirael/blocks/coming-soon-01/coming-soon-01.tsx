...
const ComingSoon = ({
  ...
  return (
    <section
      ...
      className={cn(
        "relative isolate flex min-h-svh items-center justify-center bg-background py-20",
        className,
      )}
      {...props}
    />
...
export interface ComingSoonCountdownProps extends Omit<
...
const ComingSoonCountdown = ({
  ...
  return (
    <div
      data-slot="coming-soon-countdown"
...
export interface ComingSoonFormProps extends Omit<
  ...
  if (subscribed) {
    return (
      <p
        ...
        aria-live="polite"
        className={cn(
          "inline-flex h-10 items-center gap-2 text-sm text-foreground",
          className,
        )}
      >
        ...
        <span className="text-muted-foreground">
          We&apos;ll email{" "}
          <span className="font-mono text-foreground">{email}</span> on launch
          day.
        </span>
      </p>
    );
      ...
      <div className="mx-auto w-full max-w-2xl px-6 md:px-10">
        <div className="flex flex-col items-start gap-6">
          <ComingSoonEyebrow>
            <span
              aria-hidden
              className="size-1.5 rounded-full bg-warm motion-safe:animate-pulse"
            />
            Launching soon
          </ComingSoonEyebrow>
          <ComingSoonTitle>Hirael Cloud opens in three weeks.</ComingSoonTitle>
          <ComingSoonDescription>
            Managed Postgres and object storage, with the same terminal-first
            console you already use for the registry.
          </ComingSoonDescription>
