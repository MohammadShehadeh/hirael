...
const useEmojiPicker = () => {
  ...
  if (!ctx) {
    throw new Error(
      "EmojiPicker compound parts must be used inside <EmojiPicker>",
    );
  }
  return ctx;
};
...
const EmojiPicker = ({
  ...
  const [category, setCategoryState] =
    React.useState<EmojiPickerCategoryId>("smileys");
  ...
  const [recentCodes, setRecentCodes] = React.useState<string[]>(() =>
    recentKey ? readRecent(recentKey) : [],
  );
  ...
  const display = React.useCallback(
    (item: EmojiItem) => applySkinTone(item, skinTone),
    [skinTone],
  );
...
interface EmojiPickerCategoriesProps extends Omit<
  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        ...
        e.currentTarget
          .querySelector<HTMLElement>(`[data-category="${tabs[next].id}"]`)
          ?.focus();
        e.preventDefault();
      }}
      {...props}
...
interface EmojiPickerItemProps extends Omit<
...
const EmojiPickerFooter = ({
  ...
  return (
    <div
      ...
      className={cn(
        "flex h-9 items-center gap-2 border-t border-border pt-2 text-xs",
        className,
      )}
      {...props}
    >
      ...
      <span
        ...
        className={cn(
          ...
          item ? "text-foreground capitalize" : "text-muted-foreground",
        )}
      >
...
interface EmojiPickerSkinToneProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
  labels?: Partial<Record<EmojiSkinTone, string>>;}

const EmojiPickerSkinTone = ({
  labels,
  className,
  ...props
  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        ...
        e.currentTarget
          .querySelector<HTMLElement>(`[data-tone="${next}"]`)
          ?.focus();
        e.preventDefault();
      }}
      {...props}
