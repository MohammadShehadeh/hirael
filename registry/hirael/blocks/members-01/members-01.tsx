...
interface MembersHeaderProps extends Omit<
      ...
      <TableCell className="px-4 py-2.5 text-end">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuSub>
              <DropdownMenuSubTrigger disabled={roleLocked}>
                Change role
              </DropdownMenuSubTrigger>
              <DropdownMenuSubContent>
                <DropdownMenuRadioGroup
          ...
          <FieldGroup className="gap-5">
            <Field className="gap-2">
              ...
              <FieldDescription className="text-xs">
                Press Enter, comma, or space after each address. Paste a list to
                add several at once.
              </FieldDescription>
            </Field>
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-2.5">
        <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
          Pending invites
          {typeof count === "number" ? (
            <span className="ms-2 text-foreground">({count})</span>
          ) : null}
        </span>
      </div>
...
interface MembersPendingItemProps extends Omit<
    ...
    >
      ...
      <span className="flex items-center gap-1">
        <Button
          ...
          className={cn(
            "min-w-20",
            resent && "text-success disabled:opacity-100",
          )}
        >
          {resent ? (
            <>
...
const Members01 = () => {
  ...
  const [pending, setPending] =
    React.useState<readonly PendingInvite[]>(PENDING);
