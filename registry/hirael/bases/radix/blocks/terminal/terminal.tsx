...
const TerminalHeader = ({ className, children, ...props }: TerminalHeaderProps) => {
  return (
    <div
      data-slot="terminal-header"
      className={cn('flex items-center gap-2 border-b border-border bg-muted/40 px-3 py-2', className)}
      {...props}
    >
...
const TerminalBody = ({ follow = true, className, children, ...props }: TerminalBodyProps) => {
  ...
  return (
    <div
      ...
      onScroll={onScroll}
      className={cn('max-h-80 overflow-auto p-3 font-mono text-xs leading-relaxed', className)}
      {...props}
    >
      {children}
    </div>
...
interface TerminalInputProps extends Omit<React.ComponentProps<'input'>, 'onSubmit' | 'value' | 'defaultValue'> {
  onSubmit?: (command: string) => void;
  symbol?: React.ReactNode;
  user?: React.ReactNode;
}

const TerminalInput = ({ onSubmit, symbol, user, className, disabled, ...props }: TerminalInputProps) => {
  ...
  return (
    <div data-slot="terminal-input" className={cn('flex items-center', className)}>
      ...
      <div className="w-full max-w-2xl">
        <Terminal>
          ...
          <TerminalBody>
            ...
            <TerminalInput user={TERMINAL_USER} onSubmit={run} aria-label="Terminal command" />
          </TerminalBody>
        </Terminal>
      </div>
    </section>
