...
import {
  ...
  AvatarUploadRemove,
  AvatarUploadTrigger,
} from '@/registry/hirael/bases/radix/components/avatar-upload';
    ...
    <div className="grid w-full max-w-xl gap-8">
      <div className="grid gap-2">
        ...
        <div className="flex items-center gap-5 rounded-md border border-border bg-card p-4">
          <AvatarUpload value={avatar} onValueChange={setAvatar} shape="circle" size={88} maxSize={2 * 1024 * 1024}>
            <AvatarUploadPreview fallback="MS" alt={t({ en: 'Profile photo', ar: 'صورة الملف الشخصي' })}>
              <AvatarUploadTrigger>{t({ en: 'Upload photo', ar: 'رفع صورة' })}</AvatarUploadTrigger>
              <AvatarUploadRemove aria-label={t({ en: 'Remove photo', ar: 'إزالة الصورة' })} />
            </AvatarUploadPreview>
            <AvatarUploadInput />
          ...
          <div className="grid gap-1">
            <p className="text-sm font-medium text-foreground">{t({ en: 'Mohammad Shehadeh', ar: 'محمد شحادة' })}</p>
      ...
      <div className="grid gap-2">
        ...
        <div className="grid gap-4 rounded-md border border-border bg-card p-4 sm:grid-cols-[auto_1fr] sm:items-center">
          ...
          >
            ...
            <AvatarUploadInput />
            <AvatarUploadCropDialog {...dialogCopy} title={t({ en: 'Crop the logo', ar: 'قص الشعار' })} />
            <AvatarUploadError messages={errorMessages} />
            <AvatarUploadTrigger variant="button" className="w-full">
              {logo ? t({ en: 'Replace', ar: 'استبدال' }) : t({ en: 'Upload logo', ar: 'رفع الشعار' })}
            </AvatarUploadTrigger>
          </AvatarUpload>
          <div className="grid gap-1">
