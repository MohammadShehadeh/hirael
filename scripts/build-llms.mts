...
const blocks = BLOCK_KIND_ORDER.flatMap((kind) =>
  BLOCKS_BY_KIND[kind].map((entry) => itemLine(entry, `${BLOCK_KIND_LABELS[kind]} block. `)),
);
...
const categoryLabels = COMPONENT_CATEGORY_ORDER.map((category) => `${category} = ${CATEGORY_LABELS[category]}`).join(
  ', ',
);
