...
const glow = (w: number, h: number) =>
  `radial-gradient(${w}% ${h}% at 50% 0%, transparent 0%, transparent 55%, color-mix(in oklch, var(--warm) 40%, transparent) 82%, color-mix(in oklch, var(--warm) 40%, transparent) 100%)`;
...
const Cta07 = () => {
  ...
  const scrollGlow = useTransform(
    ...
    ([w, h]: Array<number>) =>
      glow(
        Math.max(Math.floor(w ?? 100), 100),
        Math.max(Math.round(h ?? 100), 100),
      ),
  );
  ...
  return (
    <div
      ref={containerRef}
      data-slot="cta"
      className="bg-background px-6 py-16 md:px-10 md:py-24"
    >
      <motion.div
        data-slot="cta-glow"
        style={{ background: reduced ? STATIC_GLOW : scrollGlow }}
        className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl px-6 pb-24 pt-18"
      >
        <div
          data-slot="cta-body"
        ...
        >
          ...
          >
            Import a repo, watch its workflow appear as a graph, and run it.
            The first pipeline takes minutes, not an afternoon.
          </p>

          <div
            data-slot="cta-actions"
            className="mt-8 flex w-full flex-col items-stretch gap-3 sm:w-auto sm:flex-row"
          >
