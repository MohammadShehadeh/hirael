...
const AvatarUploadContext =
  React.createContext<AvatarUploadContextValue | null>(null);

const useAvatarUpload = () => {
  const ctx = React.useContext(AvatarUploadContext);
  if (!ctx) {
    throw new Error(
      "AvatarUpload compound parts must be used inside <AvatarUpload>",
    );
  }
  return ctx;
};
  ...
  return (
    <AvatarUploadContext.Provider value={ctx}>
      <div
        ...
        className={cn(
          "inline-grid w-fit justify-items-center gap-2",
          className,
        )}
        onDragEnter={(e) => {
          e.preventDefault();
...
interface AvatarUploadTriggerProps extends Omit<
...
type AvatarUploadInputProps = Omit<
  React.ComponentProps<"input">,
  "type" | "accept" | "onChange" | "multiple"
>;
...
const AvatarUploadRemove = ({
  className,
  children,
  onClick,
  ...props
}: AvatarUploadRemoveProps) => {
  const ctx = useAvatarUpload();
...
interface AvatarUploadCropDialogProps extends Omit<
...
const AvatarUploadCropDialog = ({
  ...
  return (
    ...
    >
      <DialogContent
        data-slot="avatar-upload-crop-dialog"
...
interface AvatarUploadErrorProps extends Omit<
...
const AvatarUploadErrorMessage = ({
  messages,
  className,
  ...props
}: AvatarUploadErrorProps) => {
  const ctx = useAvatarUpload();
