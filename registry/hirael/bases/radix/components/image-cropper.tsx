...
export interface ImageCropperProps extends Omit<React.ComponentProps<'div'>, 'ref' | 'onWheel' | 'onDoubleClick'> {
  src: string;
  alt?: string;
  aspect?: number;
  shape?: 'rect' | 'round';
...
const ImageCropper = ({
  ...
  const [internalZoom, setInternalZoom] = React.useState(() => clamp(defaultZoom, 1, maxZoom));
  ...
  const panBy = React.useCallback(
    (dx: number, dy: number) => {
      ...
      setCrop(clampCrop({ x: c.x + dx, y: c.y + dy }, zoomRef.current, frameSizeRef.current, naturalSizeRef.current));
    },
    [setCrop],
  );
  ...
  const setZoomCentered = React.useCallback((next: number) => zoomAt(next, { x: 0, y: 0 }), [zoomAt]);

  const reset = React.useCallback(() => {
    const z = clamp(defaultZoom, 1, maxZoom);
    setZoom(z);
    setCrop(clampCrop(defaultCrop ?? { x: 0, y: 0 }, z, frameSizeRef.current, naturalSizeRef.current));
    ...
    <ImageCropperContext.Provider value={ctx}>
      <div data-slot="image-cropper" className={cn('grid w-full gap-3', className)} {...props}>
        ...
        >
          ...
          {grid && dragging && (
            <div aria-hidden data-slot="image-cropper-grid" className="pointer-events-none absolute inset-0">
