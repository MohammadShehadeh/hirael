...
import {
  NativeSelect,
  NativeSelectOption,
} from "@/registry/hirael/ui/native-select";
    ...
    return {
      ...
      error:
        tokens.length === 0
          ...
          : `Expected 5 fields, got ${tokens.length}.`,
      errorField: null,
      fields,
    };
  }
...
export const formatCronField = (
  field: Pick<CronFieldValue, "mode" | "values" | "step">,
): string => {
...
export const formatCron = (
  fields: Record<CronField, Pick<CronFieldValue, "mode" | "values" | "step">>,
): string => {
...
export const describeCron = (expression: string): string => {
  ...
  if (minute.mode === "specific" && hour.mode === "specific") {
    ...
    parts.push(
      `At minute ${joinList(minute.values.map(String))} past every ${ordinal(hour.step)} hour`,
    );
  } else if (minute.mode === "step" && hour.mode === "every") {
    parts.push(
      minute.step === 1 ? "Every minute" : `Every ${minute.step} minutes`,
    );
  } else if (minute.mode === "step" && hour.mode === "specific") {
    parts.push(
      `Every ${minute.step} minutes past hour ${joinList(hour.values.map(String))}`,
    );
  } else if (minute.mode === "step" && hour.mode === "step") {
    parts.push(
      `Every ${minute.step} minutes past every ${ordinal(hour.step)} hour`,
    );
  ...
  if (dayOfMonth.mode === "specific") {
    parts.push(
      `on day ${joinList(dayOfMonth.values.map(String))} of the month`,
    );
  ...
  if (dayOfWeek.mode === "specific") {
    ...
    parts.push(
      dayOfMonth.mode === "specific" ? `and on ${label}` : `on ${label}`,
    );
...
interface CronEditorContextValue {
  ...
  setField: (
    field: CronField,
    patch: Partial<Pick<CronFieldValue, "mode" | "values" | "step">>,
  ) => void;
  disabled?: boolean;
}

const CronEditorContext = React.createContext<CronEditorContextValue | null>(
  null,
);
...
const useCronEditor = () => {
  ...
  if (!ctx) {
    throw new Error(
      "CronEditor compound parts must be used inside <CronEditor>",
    );
  }
  return ctx;
};
...
export interface CronEditorProps extends Omit<
...
interface CronEditorPresetsProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
...
const CronEditorPresets = ({
  presets = CRON_PRESETS,
  className,
  ...props
}: CronEditorPresetsProps) => {
...
interface CronEditorFieldsProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
...
interface CronEditorFieldProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
  field: CronField;
...
const CronEditorField = ({
  ...
  const toggleValue = (n: number) => {
    const next = selected.has(n)
      ? state.values.filter((v) => v !== n)
      : [...state.values, n];
  ...
  return (
    <Field
      ...
      className={cn(
        "gap-2 rounded-md border border-border bg-card p-3",
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <NativeSelectOption value="specific">
            {modeLabels.specific}
          </NativeSelectOption>
  ...
  return (
    ...
    >
      ...
      {showError && invalid && (
        <FieldError
          id={errorId}
          data-slot="cron-editor-expression-error"
          className="text-[11px]"
        >
          {ctx.parsed.error}
        </FieldError>
...
interface CronEditorPreviewProps extends Omit<
...
const CronEditorPreview = ({
  describe,
  className,
  ...props
}: CronEditorPreviewProps) => {
  const ctx = useCronEditor();
  const text = describe
    ? describe(ctx.parsed, ctx.value)
    : describeCron(ctx.value);
  return (
    <p
      ...
      className={cn(
        ...
        !ctx.parsed.valid && "text-destructive",
        className,
      )}
      {...props}
    >
...
interface CronEditorNextRunsProps extends Omit<
...
const CronEditorNextRuns = ({
  ...
  const ctx = useCronEditor();
  const stamp = React.useSyncExternalStore(
    subscribeMinute,
    getMinuteStamp,
    getServerMinuteStamp,
  );
  ...
  return (
    <ol
      data-slot="cron-editor-next-runs"
      className={cn("grid gap-1 font-mono text-[11px]", className)}
      {...props}
    >
      {runs.length === 0 ? (
        ...
        runs.map((d, i) => (
          <li
            key={d.getTime()}
            data-slot="cron-editor-next-run"
