...
const pickVersion = (byteLength: number, level: QRCodeLevel): number => {
  for (let version = 1; version <= 40; version++) {
    ...
    if (4 + countBits + byteLength * 8 <= dataCapacityBytes(version, level) * 8) {
      return version;
    }
  }
  throw new Error('QRCode: value too long for the chosen error correction level');
};

const buildCodewords = (bytes: Uint8Array, version: number, level: QRCodeLevel): number[] => {
