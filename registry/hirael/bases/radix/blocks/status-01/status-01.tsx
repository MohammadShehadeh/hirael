    ...
    updates: [
      {
        ...
        time: '14:20 UTC',
        message: 'Error rates are back to baseline. The slow query now runs against an index.',
      },
      {
      ...
      },
      {
        ...
        time: '12:30 UTC',
        message: 'A query without an index was saturating the primary database.',
      },
    ],
  },
  {
    ...
    updates: [
      {
        status: 'resolved' as const,
        time: '09:15 UTC',
        message: 'Queue times are back to normal. We added headroom to the runner pool.',
      },
      {
        ...
        <div data-slot="status-uptime" className="w-full">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">99.97% across all services over the last 90 days.</p>
            </div>
            <ul className="flex items-center gap-4">
              {(Object.keys(DOT_COLOR) as Array<DayStatus>).map((status) => (
                <li key={status} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <span aria-hidden className={cn('size-2 rounded-full', DOT_COLOR[status])} />
                  {STATUS_LABEL[status]}
                </li>
              ))}
            </ul>
