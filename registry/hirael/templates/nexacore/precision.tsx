...
const PILLARS: Pillar[] = [
  { label: "Scopes", start: 2.8, bottom: 7, items: ["conditions", "capacity", "specs", "timelines"] },
  { label: "Integrates", start: 22.4, bottom: 9.08, items: ["civil", "mechanical", "electrical", "controls"] },
  { label: "Certifies", start: 41.2, bottom: 11.16, items: ["redundancy", "testing", "compliance", "sign-offs"] },
  { label: "Activates", start: 61.1, bottom: 13.24, items: ["cutover", "runbooks", "handoff", "SLAs"] },
]
...
function PillarChip({ label, small }: { label: string; small?: boolean }) {
  return (
    ...
    >
      <BrandMark
        style={small ? { width: 16, height: 16 } : { width: "1.111vw", height: "1.111vw", minWidth: 16, minHeight: 16 }}
      />
      {label}
    </span>
  )
    ...
    <div className="relative hidden sm:block" style={{ width: "82.292vw", height: "31.94vw" }}>
      {PILLARS.map((pillar) => (
        <div
          ...
          style={{ insetInlineStart: `${pillar.start}vw`, bottom: `${pillar.bottom}vw` }}
        >
          ...
          <div className="relative" style={{ marginTop: 8 }}>
            ...
            <div
              ...
              style={{ position: "absolute", top: "0.56vw", insetInlineStart: "1.94vw", gap: 4 }}
            >
              {pillar.items.map((item) => (
                <span key={item} style={{ fontSize: 16, padding: "0.69vw 1.04vw", whiteSpace: "nowrap" }}>
                  {item}
                </span>
              ))}
            </div>
    ...
    <div className="flex w-full flex-col gap-10 sm:hidden">
      {PILLARS.map((pillar, i) => {
        ...
        return (
          <div
            key={pillar.label}
            className={end ? "flex flex-col items-end" : "flex flex-col items-start"}
          >
            ...
            >
              ...
              <div className="flex flex-col" style={{ alignItems: end ? "flex-end" : "flex-start" }}>
                {pillar.items.map((item) => (
                  <span key={item} style={{ fontSize: 14, color: "rgb(100, 80, 160)", padding: "8px 0" }}>
                    {item}
                  </span>
                ))}
              </div>
