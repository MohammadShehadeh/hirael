...
const StepperItem = ({ step, completed, disabled = false, className, ...props }: StepperItemProps) => {
  const { value, orientation } = useStepper();

  const state: StepState = completed || step < value ? 'completed' : step === value ? 'active' : 'inactive';

  const ctx = React.useMemo<StepperItemCtx>(() => ({ step, state, disabled }), [step, state, disabled]);
  ...
  return (
    <StepperItemContext.Provider value={ctx}>
      <div
        ...
        className={cn(
          'group/step flex',
          orientation === 'horizontal' ? 'items-center not-last:flex-1' : 'relative items-start gap-3 pb-8 last:pb-0',
          className,
        )}
        {...props}
      />
...
const StepperIndicator = ({ className, children, ...props }: React.ComponentProps<'span'>) => {
  ...
  return (
    <span
      ...
      {...props}
    >
      {children ?? (state === 'completed' ? <Check className="size-4" strokeWidth={2.5} /> : step)}
    </span>
  );
};

const StepperSeparator = ({ className, ...props }: React.ComponentProps<'div'>) => {
  return (
    <div
...
const StepperDescription = ({ className, ...props }: React.ComponentProps<'span'>) => {
  return (
    <span
...
export { Stepper, StepperItem, StepperTrigger, StepperIndicator, StepperSeparator, StepperTitle, StepperDescription };
