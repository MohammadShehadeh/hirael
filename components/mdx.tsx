...
export const mdxComponents: MDXComponents = {
  h2: ({ className, ...props }: React.ComponentProps<'h2'>) => (
    <h2
      className={cn('mt-8 text-lg font-medium tracking-[-0.01em] text-foreground first:mt-0', className)}
      {...props}
    />
  ),
  ...
  code: ({ className, ...props }: React.ComponentProps<'code'>) => (
    <code
      className={cn('rounded-sm border border-border bg-card px-1.5 py-0.5 font-mono text-[0.85em]', className)}
      {...props}
    />
  ),
  ...
  hr: ({ className }: React.ComponentProps<'hr'>) => <Separator className={cn('my-8', className)} />,
};
