...
interface LineItem { kind: "line"; index: number; line: DiffLine}
interface GapItem { kind: "gap"; start: number; count: number}
type Item = LineItem | GapItem;

const buildItems = (lines: DiffLine[],
  context: number,
...
type SplitRow =
  | { kind: "row"; key: string; left: DiffLine | null; right: DiffLine | null }
  | GapItem;
...
const useDiffViewer = () => {
  ...
  if (!ctx) {
    throw new Error(
      "DiffViewer compound parts must be used inside <DiffViewer>",
    );
  }
  return ctx;
};
...
const DiffViewer = ({
  ...
  const [expandedGaps, setExpandedGaps] = React.useState<ReadonlySet<number>>(
    () => new Set(),
  );
  ...
  const items = React.useMemo(
    () => buildItems(lines, context, expandedGaps),
    [lines, context, expandedGaps],
  );
...
const DiffViewerTitle = ({
  ...
  return (
    <div
      ...
      className={cn(
        "flex min-w-0 items-center gap-1.5 font-sans text-sm text-foreground",
        className,
      )}
      {...props}
    >
...
export interface DiffViewerContentProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
      ...
      >
        {rows.map((row) =>
          row.kind === "gap" ? (
            ...
            <div
              key={row.key}
              data-slot="diff-viewer-split-row"
...
export interface DiffViewerLineProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
...
const DiffViewerLine = ({
  ...
  const numberClass =
    "w-10 shrink-0 select-none pe-2 text-end tabular-nums text-muted-foreground";
  return (
    <div
    ...
    >
      ...
      <span
        ...
        className={cn(
          "flex-1 pe-3",
          side ? "whitespace-pre-wrap break-all" : "whitespace-pre",
        )}
      >
        {line?.content ?? ""}
      </span>
