...
// Base UI drops `false` state values and emits `true` as a bare attribute.
// Keep Radix's `data-active="true" | "false"` so `data-[active=true]`
// selectors (here and in consumers) keep working.
const activeAttribute = {
  active: (value: boolean) => ({ 'data-active': String(value) }),
};
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          className={cn(
            ...
            side === 'left' &&
              'rtl:inset-e-0 rtl:inset-s-auto rtl:border-s rtl:border-e-0 rtl:data-ending-style:translate-x-full rtl:data-starting-style:translate-x-full',
            side === 'right' &&
              'rtl:inset-s-0 rtl:inset-e-auto rtl:border-e rtl:border-s-0 rtl:data-ending-style:-translate-x-full rtl:data-starting-style:-translate-x-full',
            className,
          )}
          style={
            {
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear ltr:-translate-x-1/2 rtl:translate-x-1/2 group-data-[side=left]:-end-4 group-data-[side=right]:start-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex',
        // Resize cursors are physical, so they swap with the writing direction.
        'ltr:in-data-[side=left]:cursor-w-resize ltr:in-data-[side=right]:cursor-e-resize',
        'rtl:in-data-[side=left]:cursor-e-resize rtl:in-data-[side=right]:cursor-w-resize',
        'ltr:[[data-side=left][data-state=collapsed]_&]:cursor-e-resize ltr:[[data-side=right][data-state=collapsed]_&]:cursor-w-resize',
...
function SidebarMenuButton({
  ...
  const button = useRender({
    ...
    stateAttributesMapping: activeAttribute,
  });
