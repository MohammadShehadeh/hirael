...
import { Callout, CalloutDescription, CalloutTitle } from '@/registry/hirael/bases/base/components/callout';
...
const CalloutDemo = () => {
  ...
  return (
    <div className="grid w-full max-w-2xl gap-4">
      ...
      <Callout variant="success" title={t({ en: 'Deploy succeeded', ar: 'نجح النشر' })}>
        {t({
          en: (
            <>
          ...
          ar: (
            <>
              الإصدار <code className="font-mono">b-29f1</code> يعمل الآن على الإنتاج.
            </>
          ),
        })}
      </Callout>

      <Callout variant="warning" title={t({ en: 'Rate limit approaching', ar: 'اقتراب حد الطلبات' })}>
        {t({
      ...
      <Callout variant="destructive" title={t({ en: 'Migration failed', ar: 'فشل الترحيل' })}>
        {t({
          en: (
            <>
              Column <code className="font-mono">users.email</code> already exists. Roll back before retrying.
            </>
          ),
          ar: (
            <>
              العمود <code className="font-mono">users.email</code> موجود بالفعل. تراجع قبل إعادة المحاولة.
            </>
          ),
        })}
      </Callout>
      ...
      <Callout variant="info" icon={false}>
        {t({
          en: (
            <>
              Title-less callouts also work. Pass <code className="font-mono">icon={`{false}`}</code> to drop the
              leading icon.
            </>
          ),
          ar: (
            <>
              التنبيهات بلا عنوان تعمل أيضًا. مرّر <code className="font-mono">icon={`{false}`}</code> لإزالة الأيقونة
              الأمامية.
            </>
          ),
        })}
