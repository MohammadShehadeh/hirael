...
const CreditCardInputContext =
  React.createContext<CreditCardInputContextValue | null>(null);

const useCreditCardInput = () => {
  const ctx = React.useContext(CreditCardInputContext);
  if (!ctx) {
    throw new Error(
      "CreditCardInput compound parts must be used inside <CreditCardInput>",
    );
  }
  return ctx;
};
...
const computeErrors = (value: CreditCardValue,
  spec: CardBrandSpec,
  now: Date,): CreditCardField[] => {
  const errors: CreditCardField[] = [];
  const numberOk =
    spec.lengths.includes(value.number.length) && luhnCheck(value.number);
...
const CreditCardInput = ({
  ...
  const register = React.useCallback(
    (field: CreditCardField, el: HTMLInputElement | null) => {
...
const CreditCardInputNumber = ({
  ...
  return (
    <div
      ...
      className={cn(
        "relative min-w-0",
        ctx.variant === "row" ? "flex-1" : undefined,
      )}
    >
      <Input
        ...
        className={cn(
          "pe-16 font-mono tracking-[0.06em]",
          ctx.variant === "row" && ROW_INPUT,
          className,
        )}
        onChange={(e) => {
          ctx.setField("number", e.target.value);
          if (
            digitsOnly(e.target.value).length >= maxLength &&
            ctx.spec.lengths.length === 1
          ) {
...
const CreditCardInputExpiry = ({
  ...
  return (
    <Input
      ...
      className={cn(
        "font-mono",
        ctx.variant === "row" && cn(ROW_INPUT, "w-20 border-s border-input"),
        className,
      )}
      onChange={(e) => {
...
const CreditCardInputCvc = ({
  ...
  return (
    <Input
      ...
      className={cn(
        "font-mono",
        ctx.variant === "row" && cn(ROW_INPUT, "w-16 border-s border-input"),
        className,
      )}
...
interface CreditCardInputBrandProps extends Omit<
  React.ComponentProps<"span">,
  "children"
> {
  labels?: Partial<Record<CardBrand, string>>;}

const CreditCardInputBrand = ({
  labels,
  className,
  ...props
}: CreditCardInputBrandProps) => {
  const ctx = useCreditCardInput();
  ...
  return (
    ...
    >
      {ctx.brand === "unknown" ? (
        <CreditCard className="size-3.5" aria-label={label} />
      ) : (
        label
      )}
    </span>
