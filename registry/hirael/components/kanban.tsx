...
interface DropTarget { columnId: string; index: number}
...
interface KanbanColumnCtx { id: string}
...
const moveCard = (value: KanbanValue,
  cardId: string,
  to: DropTarget,): KanbanValue | null => {
  ...
  const target =
    to.columnId === from.columnId ? source : next[to.columnId].slice();
...
const Kanban = ({
  ...
  const locate = React.useCallback(
    (x: number, y: number, dragging: string): DropTarget | null => {
      ...
      const ids = (valueRef.current[columnId] ?? []).filter(
        (id) => id !== dragging,
      );
      let index = 0;
      for (const id of ids) {
  ...
  const updateDrag = React.useCallback(
    (x: number, y: number) => {
      ...
      if (
        next &&
        (!prev || prev.columnId !== next.columnId || prev.index !== next.index)
      ) {
  ...
  const handlePointerMove = React.useCallback(
    (e: React.PointerEvent, id: string) => {
      ...
      dropRef.current = from
        ? { columnId: from.columnId, index: from.index }
        : null;
  ...
  const handleKeyDown = React.useCallback(
    (e: React.KeyboardEvent, id: string) => {
      ...
      if (e.key === "Escape") {
        ...
        announce(
          `Move cancelled. ${cardLabel(id)} returned to its original position.`,
        );
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const focusable =
      node?.querySelector<HTMLElement>('[data-slot="kanban-card-handle"]') ??
      node;
...
export interface KanbanColumnContentProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
...
const KanbanColumnContent = ({
  ...
  const placeholderIndex =
    drag && dropTarget?.columnId === id ? dropTarget.index : null;
...
const KanbanCard = ({
  ...
  const state: "idle" | "grabbed" =
    isDragging || grabbedId === id ? "grabbed" : "idle";
  ...
  return (
    <KanbanCardContext.Provider value={cardCtx}>
      <div
        ...
        onPointerUp={
          disabled ? undefined : (e) => handlePointerEnd(e, id, false)
        }
        onPointerCancel={
          disabled ? undefined : (e) => handlePointerEnd(e, id, true)
        }
