...
export const countryFlag = (iso2: string): string => {
  ...
  return String.fromCodePoint(
    ...Array.from(code, (c) => 0x1f1e6 + c.charCodeAt(0) - 65),
  );
};
...
const CountrySelectContext =
  React.createContext<CountrySelectContextValue | null>(null);

const useCountrySelect = () => {
  const ctx = React.useContext(CountrySelectContext);
  if (!ctx) {
    throw new Error(
      "CountrySelect compound parts must be used inside <CountrySelect>",
    );
  }
  return ctx;
};
...
export type CountrySelectProps =
  CountrySelectSingleProps | CountrySelectMultipleProps;
...
const CountrySelect = (props: CountrySelectProps) => {
  ...
  const selected = React.useMemo(
    () => (valueProp === undefined ? internal : toList(valueProp)),
    [valueProp, internal],
  );
...
const CountrySelectFlag = ({
  ...
  className,
  ...props
}: CountrySelectFlagProps) => {
  return (
    <span
      aria-hidden
      data-slot="country-select-flag"
      className={cn(
        "inline-flex w-5 shrink-0 justify-center text-base leading-none",
        className,
      )}
      {...props}
    >
...
type CountrySelectTriggerProps = Omit<
...
interface CountrySelectValueProps extends Omit<
  ...
  return (
    ...
    >
      ...
      <span className="truncate">
        {picked.length <= maxNames
          ? picked.map((c) => c.name).join(", ")
          : countLabel(picked.length)}
      </span>
    </span>
  );
};
...
const CountrySelectSearch = ({
  placeholder = "Search countries",
  className,
  ...props
}: CountrySelectSearchProps) => {
  return (
    <CommandInput
...
interface CountrySelectListProps extends Omit<
