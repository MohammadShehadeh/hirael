...
const PromptInputTextarea = ({
  ...
  const { value, setValue, submit, addFiles, disabled, textareaId } =
    usePromptInput();
...
const PromptInputActions = ({
  ...
  return (
    <div
      ...
      className={cn("me-auto flex items-center gap-1", className)}
      {...props}
    />
  );
};

interface PromptInputAttachProps extends Omit<
...
interface PromptInputAttachmentProps extends Omit<
...
const PromptInputBlock = () => {
  ...
  const handleSubmit = (
    text: string,
    { attachments }: { attachments: PromptAttachment[] },
  ) => {
          ...
          {isStreaming ? (
            ...
            <>
              ...
              <span className="truncate text-foreground">
                {lastSent.text || "(no text)"}
              </span>
              <span className="shrink-0">
                · {lastSent.attachments}{" "}
                {lastSent.attachments === 1 ? "attachment" : "attachments"} ·{" "}
                {modelLabel}
              </span>
            </>
          ) : (
