...
const StorageBreadcrumb = ({ className, children, ...props }: StorageBreadcrumbProps) => {
  ...
  return (
    <nav
      ...
      aria-label="Path"
      className={cn('flex min-w-0 items-center gap-1 font-mono text-xs text-muted-foreground', className)}
      {...props}
    >
      {items.map((child, i) => (
...
const StorageItem = ({ kind, name, size, modified, icon, className, ...props }: StorageItemProps) => {
      ...
      >
        <span
          className={cn(
            ...
            kind === 'folder' ? 'bg-accent text-foreground' : 'bg-muted text-muted-foreground',
          )}
        >
        ...
        {size ? (
          <span className="hidden w-20 shrink-0 text-end font-mono text-xs text-muted-foreground sm:block">{size}</span>
        ) : null}
        {modified ? (
    ...
    <section data-slot="storage-browser-block" className="flex w-full justify-center bg-background p-6 sm:p-10">
      <div className="w-full max-w-2xl">
        <StorageBrowser>
          <StorageBrowserHeader>
            <StorageBreadcrumb>
              <StorageBreadcrumbItem current={path.length === 0} onClick={() => setPath([])}>
                my-bucket
              </StorageBreadcrumbItem>
              {path.map((segment, i) => (
                <StorageBreadcrumbItem
            ...
            <span className="shrink-0 font-mono text-xs text-muted-foreground">{entries.length} items</span>
          </StorageBrowserHeader>
          <StorageBrowserList>
            {entries.map((entry) => (
              <StorageItem
