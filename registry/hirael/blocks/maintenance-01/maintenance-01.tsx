...
const Maintenance = ({ className, ...props }: React.ComponentProps<"section">) => {
  return (
    <section
      ...
      className={cn(
        "flex min-h-svh items-center justify-center bg-background py-20",
        className,
      )}
      {...props}
    />
...
const MaintenanceTitle = ({ className, ...props }: React.ComponentProps<"h1">) => {
  return (
    <h1
      ...
      className={cn(
        "font-serif text-5xl font-medium leading-none tracking-tight sm:text-6xl md:text-7xl",
        className,
      )}
      {...props}
    />
...
const MaintenanceDescription = ({
  ...
  return (
    <p
      ...
      className={cn(
        "max-w-md text-base text-muted-foreground sm:text-lg",
        className,
      )}
      {...props}
    />
...
const MaintenanceWindow = ({
  ...
  return (
    <div
      ...
      className={cn(
        "flex w-full flex-col gap-4 rounded-sm border border-border bg-card p-5",
        className,
      )}
      {...props}
    >
...
const MaintenanceUpdates = ({
  ...
  return (
    <div
      data-slot="maintenance-updates"
      className={cn("w-full border-t border-border pt-6", className)}
      {...props}
    >
...
const MaintenanceUpdate = ({
  ...
  return (
    <li
      ...
      className={cn(
        "flex items-baseline gap-4 border-b border-border py-3 last:border-b-0",
        className,
      )}
      {...props}
    >
...
interface Update { time: string; text: string; latest?: boolean}
      ...
      <div className="mx-auto w-full max-w-2xl px-6 md:px-10">
        <div className="flex flex-col items-start gap-6">
          ...
          <MaintenanceDescription>
            We&apos;re moving the primary database to new hardware. Nothing
            you&apos;ve saved is affected, and sign-in resumes the moment
            we&apos;re done.
          </MaintenanceDescription>

          <MaintenanceWindow progress={62} progressLabel="1h 14m elapsed">
            <MaintenanceWindowRow label="Started">
              02:00 UTC
            </MaintenanceWindowRow>
            <MaintenanceWindowRow label="Expected back">
              04:00 UTC
            </MaintenanceWindowRow>
            <MaintenanceWindowRow label="Affected">
