...
const AuditLogItem = ({ open, defaultOpen, onOpenChange, className, children, ...props }: AuditLogItemProps) => {
  return (
    <Collapsible
      render={<li data-slot="audit-log-item" className={cn('flex flex-col', className)} {...props} />}
      open={open}
...
const AuditLogTrigger = ({ className, children, ...props }: AuditLogTriggerProps) => {
  return (
    ...
    >
      <ChevronRight
        aria-hidden
        className="size-3.5 shrink-0 text-muted-foreground transition-transform duration-150 group-data-open:rotate-90 rtl:group-data-closed:rotate-180"
      />
      {children}
    </CollapsibleTrigger>
  );
