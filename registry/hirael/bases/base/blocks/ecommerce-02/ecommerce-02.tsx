...
const Ecommerce02 = () => {
  ...
  const shipping = items.length === 0 || subtotal - discount >= FREE_SHIPPING_OVER ? 0 : SHIPPING_FLAT;
        ...
        {items.length === 0 ? (
          <Empty>
            <EmptyHeader>
              ...
              <EmptyDescription>
                Everything you remove ends up here as free shelf space. Refill it with the demo order to keep exploring.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
          ...
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
            <ul className="flex flex-col border-t border-border lg:col-span-2">
              {items.map((item) => (
                <li key={item.id} className="flex items-start gap-4 border-b border-border py-5 sm:gap-5">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-1">
                    ...
                    <div className="mt-2 inline-flex w-fit items-center rounded-sm border border-border">
                      ...
                      <span className="w-8 text-center font-mono text-xs tabular-nums">{item.qty}</span>
                      <Button
                        variant="ghost"
                        size="icon"
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <span className="font-mono text-sm font-medium tabular-nums">{usd(item.price * item.qty)}</span>
                    {item.qty > 1 && (
              ...
              <CardContent className="flex flex-col gap-3">
                ...
                {promoApplied && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-mono tabular-nums text-success">−{usd(discount)}</span>
                  </div>
                )}
                {shipping > 0 && (
                ...
                <div className="flex items-baseline justify-between">
                  ...
                  <span className="font-mono text-xl font-semibold tabular-nums tracking-[-0.02em]">{usd(total)}</span>
                </div>
