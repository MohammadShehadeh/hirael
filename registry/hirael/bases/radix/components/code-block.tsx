...
const CodeBlockHeader = ({ className, children, ...props }: React.ComponentProps<'div'>) => {
  const { code, language, filename, copyable } = useCodeBlock();

  return (
    <div
      data-slot="code-block-header"
      className={cn('flex min-h-10 items-center gap-2 border-b border-border px-3 py-1', className)}
      {...props}
    >
      {children ?? (
        <>
          ...
          {copyable && <CopyButton value={code} size="sm" className="ms-auto" />}
        </>
      )}
    </div>
  );
...
const CodeBlockContent = ({ className, ...props }: React.ComponentProps<'div'>) => {
  ...
  const tokenLines = React.useMemo(() => (highlight ? tokenize(code, language) : null), [highlight, code, language]);
  ...
  return (
    <div data-slot="code-block-content" className={cn('text-sm', className)} {...props}>
      <div
                    ...
                    {line.length === 0
                      ...
                      : (tokenLines?.[index]?.map((token, tokenIndex) =>
                          token.type === 'plain' ? (
                            ...
                            <span key={tokenIndex} data-token={token.type} className={TOKEN_CLASSES[token.type]}>
                              {token.content}
                            </span>
                          ),
                        ) ?? line)}
      ...
      {collapsible && (overflowing || expanded) && (
        <div data-slot="code-block-expand" className="flex justify-center border-t border-border p-1">
          <Button
            type="button"
            variant="ghost"
            size="sm"
