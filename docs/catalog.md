...
**10 templates**. Counts come from `registry.json`; the landing page derives
...
| `aswar`           | `motion`, `lucide-react`          | Bilingual Arabic/English solar landing page for "Aswar Dar Al-Iraq": a full-bleed photo hero whose Morning/Night toggle flips the whole page between light and dark, a how-it-works step grid, image case studies, an about block with a stat strip, an open-roles list, resource cards and a testimonial wall, closing on an anchor footer. The one theme-aware, token-based, right-to-left template (no self-contained palette). |
