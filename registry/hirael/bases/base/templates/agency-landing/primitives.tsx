...
interface HiraelMarkProps {
  className?: string;
}

export const HiraelMark = ({ className }: HiraelMarkProps) => {
  return (
    <svg
...
export const RollText = ({ children }: RollTextProps) => {
  return (
    <span className="block h-[20px] overflow-hidden">
      <span className={cn('flex flex-col transition-transform duration-500 group-hover:-translate-y-1/2', EASE)}>
...
interface OrangeButtonProps extends Omit<ComponentProps<'button'>, 'children'> {
  label: string;
}

export const OrangeButton = ({ label, className, ...props }: OrangeButtonProps) => {
  return (
    <button
      type="button"
      className={cn(
...
export const PartnerIcon = ({ className }: PartnerIconProps) => {
  return (
