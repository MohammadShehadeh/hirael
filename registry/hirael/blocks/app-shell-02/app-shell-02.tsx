...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/registry/hirael/ui/tabs";
    ...
    <div className="flex flex-col gap-1 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      ...
      {field.kind === "text" && (
        <InlineEdit
          ...
          validate={(next) =>
            next.trim() === "" ? `${field.label} cannot be empty` : null
          }
          selectOnFocus
...
const AppShell02 = () => {
  ...
  const matchesBySection = React.useMemo(() => {
    return SECTIONS.reduce<Record<SectionId, readonly Field[]>>(
      (acc, s) => {
        ...
        acc[s.id] = normalized
          ? fields.filter((f) => fieldMatches(f, normalized))
          : fields;
        return acc;
      },
  ...
  const totalMatches = SECTIONS.reduce(
    (sum, s) => sum + matchesBySection[s.id].length,
    0,
  );
      ...
      <div className="container w-full py-6 sm:py-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            Manage your account, security, and workspace integrations. Changes
        ...
        <p aria-live="polite" className="sr-only">
          {normalized
            ? `${totalMatches} settings match ${query.trim()}`
          ...
          <div className="lg:col-span-9">
            {SECTIONS.map((s) => {
              ...
              const elsewhere = SECTIONS.filter(
                (other) =>
                  other.id !== s.id && matchesBySection[other.id].length > 0,
              );
              return (
