...
export const isShowcased = (entry: RegistryEntry): entry is RegistryEntryMeta =>
  SHOWCASED.has(entry);
...
export const readBuiltItems = () =>
  readdirSync(R_DIR)
    .filter((name) => name.endsWith(".json") && name !== "registry.json")
    .map((name) => {
      const file = path.join(R_DIR, name);
      return {
        file,
...
const hrefByName = new Map(
  REGISTRY.map((entry) => [entry.name, entryHref(entry)]),
);
