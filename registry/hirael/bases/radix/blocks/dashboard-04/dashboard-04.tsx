  ...
  return (
    <Card className={cn('gap-2 rounded-md py-2.5', className)}>
      ...
      <CardContent className="flex flex-1 flex-col px-2.5 pb-0">
        <div className="flex flex-1 flex-col rounded-sm border border-border bg-background p-4">{children}</div>
      </CardContent>
    </Card>
  );
};
...
const Sparkline = ({ points }: { points: readonly number[] }) => {
  ...
  return (
    <svg viewBox="0 0 100 26" preserveAspectRatio="none" aria-hidden className="mt-3 h-12 w-full">
      <polyline
        points={pts}
        fill="none"
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
          <PanelCard icon={CreditCard} label="Gross revenue" className="lg:col-span-3">
            ...
            <div className="mt-4 flex flex-1 flex-col justify-end">
              <svg viewBox="0 0 100 46" preserveAspectRatio="none" aria-hidden className="h-44 w-full sm:h-56">
                {[11, 22, 33].map((y) => (
                  <line
                    key={y}
                    x1="0"
