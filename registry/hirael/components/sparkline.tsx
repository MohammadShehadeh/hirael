...
const Sparkline = ({
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const index =
      variant === "bar"
        ? Math.min(n - 1, Math.floor((px / width) * n))
        : Math.round((px / width) * (n - 1));
  ...
  const summary =
    label ??
    (data.length
      ? `Sparkline, ${data.length} points, latest ${last.toLocaleString()}`
      : "Sparkline, no data");
  ...
  return (
    <SparklineContext.Provider value={ctx}>
      <div
        ...
        data-tone={tone}
        className={cn(
          "relative inline-block h-8 w-24 align-middle",
          TONE_CLASS[tone],
          className,
        )}
      ...
      {data.map((v, i) => {
        ...
        return (
          <rect
            ...
            fill="currentColor"
            fillOpacity={
              activeIndex === null ? 0.85 : activeIndex === i ? 1 : 0.4
            }
...
const SparklineReference = ({
  value,
  className,
  ...props
}: SparklineReferenceProps) => {
...
export interface SparklineTooltipProps extends Omit<
...
const SparklineTooltip = ({
  ...
  const { data, x, width, height, activeIndex, overlay, setInteractive } =
    useSparkline();
