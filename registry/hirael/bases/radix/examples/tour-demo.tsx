...
import { Tour, TourTrigger, type TourStep } from '@/registry/hirael/bases/radix/components/tour';
        ...
        <Tour steps={steps} labels={tourLabels} onFinish={() => setCompleted(true)}>
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              ...
              {completed && <Badge variant="outline">{t({ en: 'Tour completed', ar: 'اكتملت الجولة' })}</Badge>}
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div id="tour-demo-notifications" className="rounded-md border border-border bg-card p-4">
              ...
              <div id="tour-demo-api-keys" className="rounded-md border border-border bg-card p-4">
                <p className="text-sm font-medium">{t({ en: 'API keys', ar: 'مفاتيح API' })}</p>
                <p className="mt-1 font-mono text-xs text-muted-foreground">hr_live_••••••••3f2a</p>
              </div>
              <div id="tour-demo-danger" className="rounded-md border border-destructive/40 bg-card p-4 sm:col-span-2">
                <p className="text-sm font-medium text-destructive">{t({ en: 'Danger zone', ar: 'منطقة الخطر' })}</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t({
