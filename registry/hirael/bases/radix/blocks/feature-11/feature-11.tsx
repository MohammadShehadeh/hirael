          ...
          <TabsList variant="line" className="w-full">
            {FEATURES.map((feature, index) => (
              ...
              >
                ...
                <span
                  aria-hidden
                  className="absolute inset-y-0 start-0 w-0.5 origin-top scale-y-0 bg-foreground transition-transform duration-300 ease-out group-data-[state=active]/trigger:scale-y-100 motion-reduce:transition-none"
                />
                <span className="flex min-w-0 flex-1 items-start gap-4 px-3 py-3">
                  <span className="pt-px text-xs text-muted-foreground tabular-nums transition-colors group-data-[state=active]/trigger:text-primary">
                    {formatIndex(index)}
                  </span>
                  <span className="flex min-w-0 flex-1 flex-col">
                    <span className="text-sm font-medium">{feature.title}</span>
                    <span className="grid grid-rows-[0fr] opacity-0 transition-[grid-template-rows,opacity] duration-300 ease-out group-data-[state=active]/trigger:grid-rows-[1fr] group-data-[state=active]/trigger:opacity-100 motion-reduce:transition-none">
