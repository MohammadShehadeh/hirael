  ...
  return (
    ...
    >
      ...
      <ItemActions className="flex-col items-end self-start">
        <span className={cn('text-xs tabular-nums', hasUnread ? 'font-medium text-primary' : 'text-muted-foreground')}>
          {time}
        </span>
        {hasUnread ? (
...
const ComposerEmoji = () => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger render={<InputGroupButton size="icon-sm" aria-label="Add emoji" />}>
        <SmilePlus aria-hidden />
      </PopoverTrigger>
      <PopoverContent align="end" side="top" className="w-auto p-0" finalFocus={inputRef}>
        <EmojiPicker
          onEmojiSelect={(emoji) => {
            insertText(emoji);
            setOpen(false);
