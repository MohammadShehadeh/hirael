...
import { Check, ChevronDown, Copy, FileText, LoaderCircle, TriangleAlert, type LucideIcon } from 'lucide-react';
...
const COPY_STATE_ICONS: Record<CopyState, LucideIcon> = {
  idle: Copy,
  copying: LoaderCircle,
  copied: Check,
  error: TriangleAlert,
};
...
interface V0IconProps {
  className?: string;
}

const V0Icon = ({ className }: V0IconProps) => (
...
export const CopyPageButton = ({ name, title, className }: CopyPageButtonProps) => {
  ...
  const handleCopyMarkdown = async () => {
    ...
    resetTimer.current = setTimeout(() => setCopyState('idle'), RESET_MS);
  };
  ...
  return (
    <ButtonGroup className={className}>
      ...
      >
        <StateIcon
          className={cn(
            copyState === 'copying' && 'animate-spin motion-reduce:animate-none',
