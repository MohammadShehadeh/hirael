...
export const BASES = REGISTRY_BASES.map((name) => ({
  name,
...
export const isDefaultConfig = (config: CustomizerConfig) =>
  (Object.keys(DEFAULT_CONFIG) as (keyof CustomizerConfig)[]).every((key) => config[key] === DEFAULT_CONFIG[key]);
...
export const resolveTokens = (config: CustomizerConfig): ResolvedTokens => {
  ...
  const fontFamily = config.font === DEFAULT_CONFIG.font ? null : (FONT_BY_NAME[config.font]?.family ?? null);
