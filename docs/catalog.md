...
pages). As of the last update: **86 registry UI items** (72 standalone
components + 14 distribution-only primitives), **57 section blocks**, and
...
#### Primitives — distribution-only (14)

Items listed in `DISTRIBUTION_ONLY` ship in the registry as dependencies of
other entries but have no showcase page of their own. The whole data-table
family lives here and is showcased through the `dashboard-07` block; install
that block, or any part directly with `shadcn add`, and the pieces it needs
come with it. `data-table` is the renderer and `use-data-table` the single
search-params hook — pass it `pageCount` to drive the table server-side, or
omit it to sort, filter and page a local array in memory.
...
| `data-table`                | `data-table-pagination`, `data-table-utils`, `table`                                                                            | Renderer: takes a TanStack table instance and draws header, rows, empty state and pagination.       |
...
| `use-data-table`            | `data-table-parsers`, `data-table-utils`                                                                                        | Search-params hook: URL-synced pagination, sorting and filters; manual (server) or in-memory.       |
...
| Dashboard      | `dashboard-01`, `dashboard-02`, `dashboard-03`, `dashboard-04`, `dashboard-05`, `dashboard-06`, `dashboard-07` | Analytics and admin dashboard layouts. `dashboard-06` is a pipeline overview with stat cards, a clickable runs bar chart and a breakdown panel; `dashboard-07` is an accounts data-table section — faceted / text / range / date filters, sorting, selection and pagination, all kept in the URL.                                                                                                                                                                                                                                                                                                                                                      |
