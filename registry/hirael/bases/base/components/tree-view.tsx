...
export interface TreeViewProps extends Omit<React.ComponentProps<'div'>, 'defaultValue' | 'onChange'> {
...
const TreeView = ({ value: valueProp, defaultValue, onValueChange, className, ...props }: TreeViewProps) => {
  ...
  const ctx = React.useMemo<TreeCtx>(
    ...
    [selected, setSelected, tabbable, setTabbable, registerItem, getVisibleItems],
  );
...
const TreeItem = ({
  ...
  const { selected, setSelected, tabbable, setTabbable, registerItem, getVisibleItems } = useTreeView();
  ...
  if (hasChildren) {
    return (
      ...
      >
        <CollapsibleTrigger render={<button {...triggerProps} />}>{labelRow}</CollapsibleTrigger>
        <CollapsibleContent render={<div role="group" />}>
          <TreeDepthContext.Provider value={depth + 1}>
            <TreeParentContext.Provider value={triggerRef}>{children}</TreeParentContext.Provider>
