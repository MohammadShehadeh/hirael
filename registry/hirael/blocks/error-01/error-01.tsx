...
const ErrorPageDetails = ({
  ...
  const copyText = React.useMemo(
    () => allRows.map((r) => `${r.label}: ${r.value}`).join("\n"),
    [allRows],
  );
  ...
  return (
    <Collapsible defaultOpen={defaultOpen} asChild>
      <div
        ...
        className={cn("w-full border-t border-border pt-4", className)}
        {...props}
      >
            ...
            <dl className="flex flex-col gap-2">
              {allRows.map((row) => (
                ...
                >
                  ...
                  <dd
                    dir="ltr"
                    className="select-all font-mono tabular-nums text-foreground"
                  >
                    {row.value}
                  </dd>
                </div>
      ...
      <div className="mx-auto w-full max-w-2xl px-6 md:px-10">
        <div className="flex flex-col items-start gap-6">
          ...
          <ErrorPageDescription>
            The request hit an error we didn&apos;t expect. It&apos;s been
            logged, and retrying usually works. If it keeps happening, send us
            the details below.
          </ErrorPageDescription>
