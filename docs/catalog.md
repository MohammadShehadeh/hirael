...
| `infrazen`        | `framer-motion`, `lucide-react`   | Dark, technical web3 landing page for a decentralized infrastructure protocol: a glass navbar with a live network-status pill, a plain-language hero with a live metrics strip, a hairline-joined feature grid, a centralized-cloud-versus-Infrazen reframe, a grouped integrations grid, three-tier usage pricing, a single-open FAQ and a closing CTA with footer. Self-contained near-black palette with a single mint-emerald signal accent, Space Grotesk + JetBrains Mono type. |
