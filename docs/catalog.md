...
| `edgecloud`       | `framer-motion`, `lucide-react`   | Full-page edge-inference product landing page: a glass pill header, a centered hero over a tokenized beam-and-grid backdrop, a tabbed explainer, a capabilities bento, a global-network map with location dots and a stat strip, an interactive playground card, a use-cases grid, a three-tier GPU pricing table with currency toggle, an FAQ accordion, a warm-halo contact CTA, a more-products grid and a footer with a newsletter form. Built entirely on design tokens (light and dark), full RTL. |
