...
pages). As of the last update: **86 registry UI items** (71 standalone
components + 15 distribution-only primitives), **57 section blocks**, and
...
#### Primitives — distribution-only (15)
...
come with it. `data-table` is the renderer, `data-table-client` is the
in-memory hook (+ convenience component), `use-data-table` the server hook.
...
| `data-table-client`         | `data-table`, `data-table-toolbar`, `data-table-utils`                                                                          | `useDataTableClient` (in-memory sort / filter / page) + a `DataTableClient` convenience component.  |
