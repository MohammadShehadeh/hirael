...
const UsageDashboardHeader = ({ className, ...props }: UsageDashboardHeaderProps) => {
  return (
    <div
...
const UsageDashboardTitle = ({ className, ...props }: UsageDashboardTitleProps) => {
  return (
    <h3 data-slot="usage-dashboard-title" className={cn('text-sm font-medium text-foreground', className)} {...props} />
  );
};
...
const UsageList = ({ className, ...props }: UsageListProps) => {
  return <div data-slot="usage-list" className={cn('flex flex-col gap-4 p-4', className)} {...props} />;
};
    ...
    <section data-slot="usage-dashboard-block" className="flex w-full justify-center bg-background p-6 sm:p-10">
      <UsageDashboard className="w-full max-w-md">
        ...
        <UsageList>
          ...
          <UsageItem label="Email sends" value={5200} max={5000} caption="5.2k / 5k" />
        </UsageList>
      </UsageDashboard>
    </section>
  );
