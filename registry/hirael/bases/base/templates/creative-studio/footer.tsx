    ...
    <footer className="relative overflow-hidden bg-black pb-8 pt-20 sm:pt-28 md:pt-32">
      ...
      <div className="relative container">
          ...
          {LINK_COLUMNS.map((col) => (
            <div key={col.heading} className="flex flex-col gap-3">
              ...
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-(--cs-cream)/70 transition-colors hover:text-(--cs-cream)">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-(--cs-muted)">© 2026 Hirael. All rights reserved.</p>
          <div className="flex items-center gap-5">
            <a href="#" className="text-xs text-(--cs-muted) transition-colors hover:text-(--cs-cream)">
              Privacy
            </a>
            <a href="#" className="text-xs text-(--cs-muted) transition-colors hover:text-(--cs-cream)">
              Terms
