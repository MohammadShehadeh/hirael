...
import { ArrowDown, ChevronDown, ChevronRight, RefreshCw, Sparkles, ThumbsDown, ThumbsUp } from 'lucide-react';
...
const MessageThreadContext = React.createContext<MessageThreadContextValue | null>(null);
...
const MessageThread = ({ follow = true, threshold = 48, className, children, ...props }: MessageThreadProps) => {
  ...
  const context = React.useMemo(() => ({ atBottom, scrollToBottom }), [atBottom, scrollToBottom]);
  ...
  return (
    <MessageThreadContext.Provider value={context}>
      <div
        ...
        aria-live="polite"
        onScroll={onScroll}
        className={cn('relative flex flex-col overflow-y-auto overscroll-contain', className)}
        {...props}
      >
        <div ref={contentRef} data-slot="message-thread-content" className="flex flex-col gap-6 p-4 sm:p-6">
          {children}
        </div>
      </div>
...
const MessageAvatar = ({ className, children, ...props }: MessageAvatarProps) => {
  ...
  return (
    <span
      ...
      {...props}
    >
      {children ?? (role === 'assistant' ? <Sparkles className="size-3.5" /> : null)}
    </span>
  );
};
...
const MessageActions = ({ alwaysVisible = false, className, ...props }: MessageActionsProps) => {
  return (
    <div
...
const MessageAction = ({ label, pressed, className, children, ...props }: MessageActionProps) => {
  return (
    <Button
      type="button"
...
interface MessageToolCallProps extends Omit<React.ComponentProps<typeof Collapsible>, 'children'> {
  name: string;
...
interface MessageReasoningProps extends Omit<React.ComponentProps<typeof Collapsible>, 'children'> {
...
const MessageReasoning = ({
  ...
  const text = label ?? (isThinking ? 'Thinking' : duration != null ? `Thought for ${duration}s` : 'Reasoning');
...
const MessageTyping = ({ label = 'Assistant is typing', className, ...props }: MessageTypingProps) => {
  return (
    <div
...
const MessageSources = ({ label = 'Sources', className, children, ...props }: MessageSourcesProps) => {
  return (
    <div
        ...
        {count === 0 ? (
          ...
          <MessageContent aria-busy={!done}>
            {paragraphs.map((paragraph, i) => (
              <p key={i}>
                {paragraph}
                {!done && i === paragraphs.length - 1 ? <MessageStreamingCursor /> : null}
              </p>
            ))}
          </MessageContent>
        )}
      ...
      <div className="flex w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-border">
        ...
        <MessageThread className="h-[520px]">
          ...
          <Message role="user">
            ...
            <MessageBody>
              <MessageContent>
                Our API p95 latency has been climbing since Tuesday. Any idea what changed?
              </MessageContent>
              <MessageTimestamp dateTime="2026-08-25T10:41:00Z">10:41</MessageTimestamp>
            </MessageBody>
          </Message>
          ...
          <Message role="assistant">
            ...
            <MessageBody className="gap-2">
              <MessageReasoning duration={4}>
                <p>
                  The user is asking about a latency regression. Pull the last seven days of p95 for the api service and
                  compare it with the deploy history before guessing at a cause.
                </p>
              </MessageReasoning>
              <MessageToolCall
                name="query_metrics"
          ...
          <Message role="user">
            <MessageAvatar>MS</MessageAvatar>
            <MessageBody>
              <MessageContent>Draft a short update for the incident channel.</MessageContent>
              <MessageTimestamp dateTime="2026-08-25T10:43:00Z">10:43</MessageTimestamp>
            </MessageBody>
          </Message>
