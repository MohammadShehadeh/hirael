  ...
  return (
    ...
    >
      <span className="relative flex size-1.5">
        ...
        <span
          className={cn('relative inline-flex size-1.5 rounded-full', statusTone[status].split(' ')[0])}
          aria-hidden
        />
      </span>
      {children ?? status}
...
const ServerCardSpecs = ({ className, ...props }: ServerCardSpecsProps) => {
  return <dl data-slot="server-card-specs" className={cn('grid grid-cols-3 gap-3', className)} {...props} />;
};
...
const ServerCardMeter = ({ label, value, thresholds = [0.75, 0.9], className, ...props }: ServerCardMeterProps) => {
