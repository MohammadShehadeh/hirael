...
import {
  ...
  JsonViewerExpandAll,
  JsonViewerTree,
} from "@/registry/hirael/components/json-viewer";
      ...
      <div className="grid gap-2">
        ...
        <JsonViewer value={CONFIG} defaultExpanded={1} className="p-0">
          <div className="flex items-center justify-between gap-2 border-b border-border px-2 py-1">
            <span className="ps-1 font-sans text-xs text-muted-foreground">
              package.json
            </span>
            <div className="flex items-center gap-1">
              <JsonViewerExpandAll>
                {t({ en: "Expand all", ar: "توسيع الكل" })}
              </JsonViewerExpandAll>
              <JsonViewerCollapseAll>
                {t({ en: "Collapse all", ar: "طي الكل" })}
              </JsonViewerCollapseAll>
              <JsonViewerCopy />
            </div>
          </div>
